    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                // Posterize to discrete bands
                // Con N steps tenemos N niveles: 0, 1/(N-1), 2/(N-1), ... 1
                float stepped = floor(lightIntensity * _ShadowSteps) / (_ShadowSteps - 1.0);
                stepped = saturate(stepped); // Clamp entre 0 y 1
                
                // Apply light color with stepped intensity
                float3 lighting = mainLight.color * stepped;
                
                // Add ambient light so shadows aren't pure black
